...
df_r134a = pd.read_csv("../csv/r134a-pareto-iter2.csv", index_col=0)
df_r134a = df_r134a.loc[33]
print(df_r134a)
df_r134a_lit = pd.read_csv("../csv/r134a-lit.csv", index_col=False)

def main():

    #fig, ax2 = plt.subplots(1, 1, figsize=(6,6))
    temps_r134a = R134a.expt_liq_density.keys()

    '''clrs = seaborn.color_palette('bright', n_colors=len(df_r134a))
    np.random.seed(11)
    np.random.shuffle(clrs)

    for temp in temps_r134a:
        ax2.scatter(
            df_r134a.filter(regex=(f"liq_density_{float(temp):.0f}K")),
            np.tile(temp, len(df_r134a)),
            c=clrs,
            s=160,
            alpha=0.2,
        )
        ax2.scatter(
            df_r134a.filter(regex=(f"vap_density_{float(temp):.0f}K")),
            np.tile(temp, len(df_r134a)),
            c=clrs,
            s=160,
            alpha=0.2,
    ...
    ax2.scatter(
        df_r134a.filter(regex=("sim_rhoc")),
        df_r134a.filter(regex=("sim_Tc")),
        c=clrs,
        s=160,
        alpha=0.2,
    )

    tc, rhoc = calc_critical(df_r134a_gaff)
    ax2.scatter(
        df_r134a_gaff["liq_density"],
        df_r134a_gaff["temperature"],
        c='gray',
        s=120,
        alpha=0.7,
        marker='s',
    ...
    tc, rhoc = calc_critical(df_r134a_lit)
    ax2.scatter(
        df_r134a_lit["liq_density"],
        df_r134a_lit["temperature"],
        c='#0989d9',
        s=160,
        alpha=0.7,
        marker='^',
    ...
    ax2.scatter(
        R134a.expt_liq_density.values(),
        R134a.expt_liq_density.keys(),
        color="black",
        marker="x",
        linewidths=2,
        s=200,
    ...
    ax2.scatter(
        R134a.expt_vap_density.values(),
        R134a.expt_vap_density.keys(),
        color="black",
        marker="x",
        linewidths=2,
        s=200,
    )
    ax2.scatter(R134a.expt_rhoc, R134a.expt_Tc, color="black", marker="x", linewidths=2, s=200)

    ax2.set_xlim(-134a, 18134a)
    ax2.xaxis.set_major_locator(MultipleLocator(134a0))
    ...
    ax2.text(0.7,  0.82, "R-134a", fontsize=30, transform=ax2.transAxes)
    fig.subplots_adjust(bottom=0.2, top=0.75, left=0.15, right=0.95, wspace=0.55)

    fig.savefig("pdfs/fig3_r134a-results-vle.png",dpi=300)'''
    ...
    for temp in temps_r134a:
        axs[0].scatter(
            temp,
            df_r134a.loc[f"sim_Pvap_{float(temp):.0f}K"],
            #np.tile(temp, len(df_r134a)),
            #df_r134a.filter(regex=(f"Pvap_{float(temp):.0f}K")),
            c='blue',
            s=70,
            alpha=0.7,
        )
    ...
    '''axs[0].scatter(
        df_r134a_trappe["temperature"],
        df_r134a_trappe["Pvap"],
        c='red',
        s=70,
        alpha=0.7,
        label="TraPPE",
    ...
    axs[0].scatter(
        R134a.expt_Pvap.keys(),
        R134a.expt_Pvap.values(),
        color="black",
        marker="x",
        label="Experiment",
        s=80,
    ...
    for temp in temps_r134a:
        axs[1].scatter(
            temp,
            df_r134a.loc[f"sim_Hvap_{float(temp):.0f}K"],
            #np.tile(temp, len(df_r134a)),
            #df_r134a.filter(regex=(f"Hvap_{float(temp):.0f}K")),
            c='blue',
            s=70,
            alpha=0.7,
        )
    '''axs[1].scatter(
        df_r134a_gaff["temperature"],
        df_r134a_gaff["Hvap"] / R134a.molecular_weight * 1000.0,
        c='gray',
        s=70,
        alpha=0.7,
        marker='s',
    ...
    axs[1].scatter(
        df_r134a_lit["temperature"],
        df_r134a_lit["Hvap"] /R134a.molecular_weight * 1000.0, #kj/mol to kj/kg
        c='#0989d9',
        s=70,
        alpha=0.7,
        marker='^',
    )
    print(df_r134a_lit["temperature"],df_r134a_lit["Hvap"] /R134a.molecular_weight * 1000.0)
    '''axs[1].scatter(
        df_r134a_trappe["temperature"],
        df_r134a_trappe["Hvap"] ,#kj/mol
        c='red',
        s=70,
        alpha=0.7,
        marker='*',
    ...
    axs[1,1].scatter(
        df_r134a_gaff["T_K"],
        df_r134a_gaff["hvap_kJmol"] / R134a.molecular_weight * 1000.0,
        c='gray',
        s=134a,
        alpha=0.7,
        marker='s',
    ...
    axs[0].text(0.08, 0.8, "HFC-134a", fontsize=20, transform=axs[0].transAxes)
    ...
    axs[1,0].text(0.56, 0.08, "R-134a", fontsize=16, transform=axs[1,0].transAxes)

    axs[1,1].text(0.08, 0.8, "d", fontsize=20, transform=axs[1,1].transAxes)
    axs[1,1].text(0.5, 0.8, "HFC-134a", fontsize=16, transform=axs[1,1].transAxes)'''


    axs[0].legend(loc="lower left", bbox_to_anchor=(0.7, 1.05), ncol=2, fontsize=16, handletextpad=0.1, markerscale=0.8, edgecolor="dimgrey")
