...
print(df_r134a)
df_r134a_gaff = pd.read_csv("../csv/r134a-lit.csv", index_col=False) #https://pubs.acs.org/doi/abs/10.1021/jp806213w FF#2
...
def main():
    ...
    for temp in temps_r134a:
        print(df_r134a.loc[f"sim_liq_density_{float(temp):.0f}K"])
        ax2.scatter(
            df_r134a.loc[f"sim_liq_density_{float(temp):.0f}K"],
            temp,
            c='blue',
            s=160,
            alpha=0.7,
        ...
        ax2.scatter(
            df_r134a.loc[f"sim_vap_density_{float(temp):.0f}K"],
            temp,
            c='blue',
            s=160,
            alpha=0.7,
    ...
    ax2.legend(loc="lower left", bbox_to_anchor=(-0.18, 1.03), ncol=2, fontsize=22, handletextpad=0.1, markerscale=0.9, edgecolor="dimgrey")
    ...
    ax2.text(0.5,  0.82, "HFC-134a", fontsize=30, transform=ax2.transAxes)
...
'''    # Plot Pvap / Hvap
    ...
    axs[0,1].text(0.5, 0.8, "HFC-134a", fontsize=16, transform=axs[0,1].transAxes)
