...
dfs = [prepare_df_vle(df_csv, R134a) for df_csv in df_csvs]


def main():

    # Create a dataframe with one row per parameter set
    dfs_paramsets = [prepare_df_vle_errors(df, R134a) for df in dfs]
    ...
    text.set_y(3)
    axes[0].set_title(r"HFC-134a",fontsize=16)
    ...
    fig.savefig("pdfs/fig2_r134a-cumu-vle.png",dpi=300)
