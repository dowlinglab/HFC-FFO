...
param_csv_names = [
    "r134a-density-iter" + str(i) + "-params.csv" for i in range(1, iternum + 1)
]
print(param_csv_names)
result_csv_names = [
    "r134a-density-iter" + str(i) + "-results.csv" for i in range(1, iternum + 1)
]
df_params = [
...
df_results["expt_density"] = df_results["temperature"].apply(
    lambda x: R134a.expt_liq_density[int(x)]
)
df_results["sq_err"] = (df_results["density"] - df_results["expt_density"]) ** 2
df_mse = (
    df_results.groupby(list(R134a.param_names))["sq_err"].mean().reset_index(name="mse")
)
...
for params1 in scaled_param_values:
    for idx, params2 in enumerate(df_params[list(R134a.param_names)].values):
        if np.allclose(params1, params2):
            param_idxs.append(idx)
            param_vals.append(params2)
            break
df_mse["param_idx"] = param_idxs
df_mse[list(R134a.param_names)] = param_vals
...
distance_opt_l,number_points_l = bisection(lower_bound, upper_bound, error_tol, top_param_set, R134a, target_num_l, dist_seed)
print('\nRequired Distance for liquid is : %0.8f and there are %0.1f points too few' % (distance_opt_l, number_points_l) )

new_points_vle = opt_dist(distance_opt_l, top_param_set, R134a, target_num_l, rand_seed=dist_seed , eval = True)
