...
def main():
    ...
    plot_property(
        df_all,
        "vap_density",
        R134a.vap_density_bounds,
        axis_name="Vapor Density [kg/m$^3$]",
    )
    plot_property(
        df_all, "Pvap", R134a.Pvap_bounds, axis_name="Vapor Pressure [bar]"
    )
    plot_property(
        df_all,
        "Hvap",
    ...
    d = pd.DataFrame({'T':list(values_scaled_to_real(df_all["temperature"],R134a.temperature_bounds)) , 'sim_Pvap':list(values_scaled_to_real(df_all["sim_Pvap"], R134a.Pvap_bounds)) }, columns=['T', 'sim_Pvap'])
    print(d.loc[d['T'] == 240])
    ...
    temps = R134a.expt_liq_density.keys()
    for temp in temps:
        ax.scatter(
    ...
    ax.scatter(R134a.expt_rhoc, R134a.expt_Tc, color="black", marker="x", s=80)
