...
df_r143a = pd.read_csv("../csv/r143a-pareto.csv", index_col=0)
df_r143a = df_r143a.loc[65]
print(df_r143a)
df_r143a_gaff = pd.read_csv("../../gaff/results.csv", index_col=False)

def main():

    #fig, ax2 = plt.subplots(1, 1, figsize=(6,6))
    temps_r143a = R143a.expt_liq_density.keys()

    '''clrs = seaborn.color_palette('bright', n_colors=len(df_r143a))
    np.random.seed(11)
    np.random.shuffle(clrs)

    for temp in temps_r143a:
        ax2.scatter(
            df_r143a.filter(regex=(f"liq_density_{float(temp):.0f}K")),
            np.tile(temp, len(df_r143a)),
            c=clrs,
            s=160,
            alpha=0.2,
        )
        ax2.scatter(
            df_r143a.filter(regex=(f"vap_density_{float(temp):.0f}K")),
            np.tile(temp, len(df_r143a)),
            c=clrs,
            s=160,
            alpha=0.2,
    ...
    ax2.scatter(
        df_r143a.filter(regex=("sim_rhoc")),
        df_r143a.filter(regex=("sim_Tc")),
        c=clrs,
        s=160,
        alpha=0.2,
    )

    tc, rhoc = calc_critical(df_r143a_gaff)
    ax2.scatter(
        df_r143a_gaff["liq_density"],
        df_r143a_gaff["temperature"],
        c='gray',
        s=120,
        alpha=0.7,
        marker='s',
    ...
    tc, rhoc = calc_critical(df_r143a_lit)
    ax2.scatter(
        df_r143a_lit["liq_density"],
        df_r143a_lit["temperature"],
        c='#0989d9',
        s=160,
        alpha=0.7,
        marker='^',
    ...
    ax2.scatter(
        R143a.expt_liq_density.values(),
        R143a.expt_liq_density.keys(),
        color="black",
        marker="x",
        linewidths=2,
        s=200,
    ...
    ax2.scatter(
        R143a.expt_vap_density.values(),
        R143a.expt_vap_density.keys(),
        color="black",
        marker="x",
        linewidths=2,
        s=200,
    )
    ax2.scatter(R143a.expt_rhoc, R143a.expt_Tc, color="black", marker="x", linewidths=2, s=200)

    ax2.set_xlim(-143a, 18143a)
    ax2.xaxis.set_major_locator(MultipleLocator(143a0))
    ...
    ax2.text(0.7,  0.82, "R-143a", fontsize=30, transform=ax2.transAxes)
    fig.subplots_adjust(bottom=0.2, top=0.75, left=0.15, right=0.95, wspace=0.55)

    fig.savefig("pdfs/fig3_r143a-results-vle.png",dpi=300)'''
    ...
    #np.random.shuffle(clrs)
    for temp in temps_r143a:
        print(df_r143a.loc[f"sim_Pvap_{float(temp):.0f}K"])
        axs[0].scatter(
            temp,
            df_r143a.loc[f"sim_Pvap_{float(temp):.0f}K"],
            #np.tile(temp, len(df_r143a)),
            #df_r143a.filter(regex=(f"Pvap_{float(temp):.0f}K")),
            c='blue',#clrs,
            s=70,
            alpha=0.7,
        )
    ...
    axs[0].scatter(
        df_r143a_trappe["temperature"],
        df_r143a_trappe["Pvap"],
        c='red',
        s=70,
        alpha=0.7,
        label="TraPPE",
    ...
    axs[0].scatter(
        R143a.expt_Pvap.keys(),
        R143a.expt_Pvap.values(),
        color="black",
        marker="x",
        label="Experiment",
        s=80,
    ...
    for temp in temps_r143a:
        axs[1].scatter(
            temp,
            df_r143a.loc[f"sim_Hvap_{float(temp):.0f}K"],
            #np.tile(temp, len(df_r143a)),
            #df_r143a.filter(regex=(f"Hvap_{float(temp):.0f}K")),
            c='blue',
            s=70,
            alpha=0.7,
        )
    axs[1].scatter(
        df_r143a_gaff["temperature"],
        df_r143a_gaff["Hvap"] / R143a.molecular_weight * 1000.0,
        c='gray',
        s=70,
        alpha=0.7,
        marker='s',
    )
    print(df_r143a_gaff["temperature"],df_r143a_gaff["Hvap"] / R143a.molecular_weight * 1000.0,)
    '''axs[1].scatter(
        df_r143a_lit["temperature"],
        df_r143a_lit["Hvap"] ,#kj/kg
        c='#0989d9',
        s=70,
        alpha=0.7,
        marker='^',
    ...
    axs[1].scatter(
        R143a.expt_Hvap.keys(),
        R143a.expt_Hvap.values(),
        color="black",
        marker="x",
        s=80,
    )
    ...
    axs[1,1].scatter(
        df_r143a_gaff["T_K"],
        df_r143a_gaff["hvap_kJmol"] / R143a.molecular_weight * 1000.0,
        c='gray',
        s=143a,
        alpha=0.7,
        marker='s',
    ...
    axs[0].text(0.08, 0.8, "HFC-143a", fontsize=20, transform=axs[0].transAxes)
    ...
    axs[1,0].text(0.56, 0.08, "R-143a", fontsize=16, transform=axs[1,0].transAxes)

    axs[1,1].text(0.08, 0.8, "d", fontsize=20, transform=axs[1,1].transAxes)
    axs[1,1].text(0.5, 0.8, "HFC-143a", fontsize=16, transform=axs[1,1].transAxes)'''


    axs[0].legend(loc="lower left", bbox_to_anchor=(0.75, 1.05), ncol=2, fontsize=16, handletextpad=0.1, markerscale=0.8, edgecolor="dimgrey")
