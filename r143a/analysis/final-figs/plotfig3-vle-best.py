...
print(df_r143a)
df_r143a_gaff = pd.read_csv("../csv/r143a-gaff.csv", index_col=False)
...
def main():
    ...
    for temp in temps_r143a:
        print(df_r143a.loc[f"sim_liq_density_{float(temp):.0f}K"])
        ax2.scatter(
            df_r143a.loc[f"sim_liq_density_{float(temp):.0f}K"],
            temp,
            c='blue',
            s=160,
            alpha=0.7,
        ...
        ax2.scatter(
            df_r143a.loc[f"sim_vap_density_{float(temp):.0f}K"],
            temp,
            c='blue',
            s=160,
            alpha=0.7,
    ...
    ax2.legend(loc="lower left", bbox_to_anchor=(-0.02, 1.03), ncol=2, fontsize=22, handletextpad=0.1, markerscale=0.9, edgecolor="dimgrey")
    ...
    ax2.text(0.5,  0.82, "HFC-143a", fontsize=30, transform=ax2.transAxes)
...
'''    # Plot Pvap / Hvap
    ...
    axs[0,1].text(0.5, 0.8, "HFC-143a", fontsize=16, transform=axs[0,1].transAxes)
