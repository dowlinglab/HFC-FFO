...
df_r170 = pd.read_csv("../csv/r170-pareto-iter2.csv", index_col=0)
df_r170 = df_r170.loc[42]
print(df_r170)
df_r170_gaff = pd.read_csv("../../run/gaff/results.csv", index_col=False)
# Add https://doi.org/10.1021/jp9072137
df_r170_lit = pd.read_csv("../csv/r170-lit.csv", index_col=False)
df_r170_trappe = pd.read_csv("../csv/r170-trappe.csv", index_col=False)

def main():

    #fig, ax2 = plt.subplots(1, 1, figsize=(6,6))
    temps_r170 = R170.expt_liq_density.keys()

    '''clrs = seaborn.color_palette('bright', n_colors=len(df_r170))
    np.random.seed(11)
    np.random.shuffle(clrs)

    for temp in temps_r170:
        ax2.scatter(
            df_r170.filter(regex=(f"liq_density_{float(temp):.0f}K")),
            np.tile(temp, len(df_r170)),
            c=clrs,
            s=160,
            alpha=0.2,
        )
        ax2.scatter(
            df_r170.filter(regex=(f"vap_density_{float(temp):.0f}K")),
            np.tile(temp, len(df_r170)),
            c=clrs,
            s=160,
            alpha=0.2,
    ...
    ax2.scatter(
        df_r170.filter(regex=("sim_rhoc")),
        df_r170.filter(regex=("sim_Tc")),
        c=clrs,
        s=160,
        alpha=0.2,
    )

    tc, rhoc = calc_critical(df_r170_gaff)
    ax2.scatter(
        df_r170_gaff["liq_density"],
        df_r170_gaff["temperature"],
        c='gray',
        s=120,
        alpha=0.7,
        marker='s',
    ...
    tc, rhoc = calc_critical(df_r170_lit)
    ax2.scatter(
        df_r170_lit["liq_density"],
        df_r170_lit["temperature"],
        c='#0989d9',
        s=160,
        alpha=0.7,
        marker='^',
    ...
    ax2.scatter(
        R170.expt_liq_density.values(),
        R170.expt_liq_density.keys(),
        color="black",
        marker="x",
        linewidths=2,
        s=200,
    ...
    ax2.scatter(
        R170.expt_vap_density.values(),
        R170.expt_vap_density.keys(),
        color="black",
        marker="x",
        linewidths=2,
        s=200,
    )
    ax2.scatter(R170.expt_rhoc, R170.expt_Tc, color="black", marker="x", linewidths=2, s=200)

    ax2.set_xlim(-170, 18170)
    ax2.xaxis.set_major_locator(MultipleLocator(1700))
    ...
    ax2.text(0.7,  0.82, "R-170", fontsize=30, transform=ax2.transAxes)
    fig.subplots_adjust(bottom=0.2, top=0.75, left=0.15, right=0.95, wspace=0.55)

    fig.savefig("pdfs/fig3_r170-results-vle.png",dpi=300)'''
    ...
    for temp in temps_r170:
        axs[0].scatter(
            temp,
            df_r170.loc[f"sim_Pvap_{float(temp):.0f}K"],
            #np.tile(temp, len(df_r170)),
            #df_r170.filter(regex=(f"Pvap_{float(temp):.0f}K")),
            c='blue',
            s=70,
            alpha=0.7,
        )
    ...
    for temp in temps_r170:
        axs[1].scatter(
            temp,
            df_r170.loc[f"sim_Hvap_{float(temp):.0f}K"],
            #np.tile(temp, len(df_r170)),
            #df_r170.filter(regex=(f"Hvap_{float(temp):.0f}K")),
            c='blue',
            s=70,
            alpha=0.7,
        )
    axs[1].scatter(
        df_r170_gaff["temperature"],
        df_r170_gaff["Hvap"] / R170.molecular_weight * 1000.0,
        c='gray',
        s=70,
        alpha=0.7,
        marker='s',
    )
    print(df_r170_gaff["temperature"],df_r170_gaff["Hvap"] / R170.molecular_weight * 1000.0)
    axs[1].scatter(
        df_r170_lit["temperature"],
        df_r170_lit["Hvap"] ,#kj/kg
        c='#0989d9',
        s=70,
        alpha=0.7,
        marker='^',
    ...
    axs[1,1].scatter(
        df_r170_gaff["T_K"],
        df_r170_gaff["hvap_kJmol"] / R170.molecular_weight * 1000.0,
        c='gray',
        s=170,
        alpha=0.7,
        marker='s',
    ...
    axs[0].text(0.08, 0.8, "R-170", fontsize=20, transform=axs[0].transAxes)
    ...
    axs[1,0].text(0.56, 0.08, "R-170", fontsize=16, transform=axs[1,0].transAxes)

    axs[1,1].text(0.08, 0.8, "d", fontsize=20, transform=axs[1,1].transAxes)
    axs[1,1].text(0.5, 0.8, "HFC-170", fontsize=16, transform=axs[1,1].transAxes)'''
