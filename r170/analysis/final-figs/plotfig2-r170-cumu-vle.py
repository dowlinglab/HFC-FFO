...
dfs = [prepare_df_vle(df_csv, R170) for df_csv in df_csvs]


def main():

    # Create a dataframe with one row per parameter set
    dfs_paramsets = [prepare_df_vle_errors(df, R170) for df in dfs]
    ...
    text.set_y(3)
    axes[0].set_title(r"R-170",fontsize=16)
    ...
    fig.savefig("pdfs/fig2_r170-cumu-vle.png",dpi=300)
