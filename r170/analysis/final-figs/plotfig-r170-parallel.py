...
def main():
    ...
    df = pd.read_csv("../csv/r170-pareto-iter2.csv", index_col=0)
    dff = pd.read_csv("../csv/r170-final-iter2.csv", index_col=0)

    seaborn.set_palette('bright', n_colors=len(df))
    data = df[list(R170.param_names)].values
    ...
    for i in range(df_gaff["temperature"].shape[0]):
        ape=[]
        ape.append(np.abs(df_gaff["liq_density"][i]-R170.expt_liq_density[int(df_gaff["temperature"][i])])/R170.expt_liq_density[int(df_gaff["temperature"][i])])
    mape_gaff.append(np.mean(ape))
    for i in range(df_gaff["temperature"].shape[0]):
        ape=[]
        ape.append(np.abs(df_gaff["vap_density"][i]-R170.expt_vap_density[int(df_gaff["temperature"][i])])/R170.expt_vap_density[int(df_gaff["temperature"][i])])
    mape_gaff.append(np.mean(ape))
    for i in range(df_gaff["temperature"].shape[0]):
        ape=[]
        ape.append(np.abs(df_gaff["Pvap"][i]-R170.expt_Pvap[int(df_gaff["temperature"][i])])/R170.expt_Pvap[int(df_gaff["temperature"][i])])
    mape_gaff.append(np.mean(ape))
    for i in range(df_gaff["temperature"].shape[0]):
        ape=[]
        ape.append(np.abs(df_gaff["Hvap"][i]-R170.expt_Hvap[int(df_gaff["temperature"][i])]*R170.molecular_weight/1000)/(R170.expt_Hvap[int(df_gaff["temperature"][i])]*R170.molecular_weight/1000)) #convert j/g to kj/mol for experimental values in R170 constants
    mape_gaff.append(np.mean(ape))
    ...
    ax.plot(x_vals[-1], df_lit.loc["r170"][-1]/bounds[-1][1], markersize=12, color="#0989d9", marker="^", alpha=0.5, clip_on=False, zorder=200,label="Potoff et al.")
    ax.plot(x_vals[-2], df_lit.loc["r170"][-2]/bounds[-2][1], markersize=12, color="#0989d9", marker="^", alpha=0.5, clip_on=False, zorder=200)
    ax.plot(x_vals[-3], df_lit.loc["r170"][-3]/bounds[-3][1], markersize=12, color="#0989d9", marker="^", alpha=0.5, clip_on=False, zorder=200)
    ax.plot(x_vals[-4], df_lit.loc["r170"][-4]/bounds[-4][1], markersize=12, color="#0989d9", marker="^", alpha=0.5, clip_on=False, zorder=200)
    # Add TraPPE
    ax.plot(x_vals[-1], df_lit.loc["r170_trappe"][-1]/bounds[-1][1], markersize=12, color="red", marker="*", alpha=0.5, clip_on=False, zorder=200,label="TraPPE")
    ax.plot(x_vals[-2], df_lit.loc["r170_trappe"][-2]/bounds[-2][1], markersize=12, color="red", marker="*", alpha=0.5, clip_on=False, zorder=200)
    ax.plot(x_vals[-3], df_lit.loc["r170_trappe"][-3]/bounds[-3][1], markersize=12, color="red", marker="*", alpha=0.5, clip_on=False, zorder=200)
    ax.plot(x_vals[-4], df_lit.loc["r170_trappe"][-4]/bounds[-4][1], markersize=12, color="red", marker="*", alpha=0.5, clip_on=False, zorder=200)
    ...
    fig.savefig("pdfs/fig_r170-parallel.png",dpi=360)
