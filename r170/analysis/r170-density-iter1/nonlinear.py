...
from models import run_gpflow_scipy
...
#combine sigma and epsilon by lowering the dimension
def nonlinear(original_array, total_new_columns):
    new_array = np.zeros((original_array.shape[0],total_new_columns))
    new_array[:,0] = original_array[:,0] * original_array[:,2]
    new_array[:,1] = original_array[:,1] * original_array[:,3]
    new_array[:,2] = original_array[:,4] 
    return new_array
def nonlinear_noT(original_array, total_new_columns):
    new_array = np.zeros((original_array.shape[0],total_new_columns))
    new_array[:,0] = original_array[:,0] * original_array[:,2]
    new_array[:,1] = original_array[:,1] * original_array[:,3]
    return new_array
...
liquid_density_threshold = 320  # kg/m^3  ##>500 is liquid; <500 is gas. used for classifier

csv_path = "/scratch365/nwang2/ff_development/HFC_143a_FFO_FF/r170/analysis/csv/"
in_csv_names = ["r170-density-iter" + str(i) + "-results.csv" for i in range(1, iternum+1)]
out_csv_name = "r170-density-iter" + str(iternum + 1) + "-params.csv"
out_top_liquid_csv_name = "r170-density-iter" + str(iternum ) + "-liquid-params.csv"
out_top_vapor_csv_name = "r170-density-iter" + str(iternum ) + "-vapor-params.csv"
...
print(liquid_samples)
ranked_liquid_samples = rank_samples(liquid_samples, model, R170, "sim_liq_density")
ranked_vapor_samples = rank_samples(vapor_samples, model, R170, "sim_liq_density")#both l and g compared to liquid density
...
dist_seed = 115
target_num_v = 20
target_num_l = 180 # just for liquid; vapor less than 100 and use all
...
distance_opt_v,number_points_v = bisection(lower_bound, upper_bound, error_tol, top_vap, R170, target_num_v, dist_seed)
print('\nRequired Distance for vapor is : %0.8f and there are %0.1f points too few' % (distance_opt_v, number_points_v) )

distance_opt_l,number_points_l = bisection(lower_bound, upper_bound, error_tol, top_liq, R170, target_num_l, dist_seed)
print('\nRequired Distance for liquid is : %0.8f and there are %0.1f points too few' % (distance_opt_l, number_points_l) )

new_points_l = opt_dist(distance_opt_l, top_liq, R170, target_num_l, rand_seed=dist_seed , eval = True)
...
new_points_v = opt_dist(distance_opt_v, top_vap, R170, target_num_v, rand_seed=dist_seed , eval = True)
