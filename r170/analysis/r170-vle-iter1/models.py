...
def run_gpflow_scipy(x_train, y_train, kernel, mean_function="linear", fmt="notebook"):
    ...
    if mean_function is not None:
        ...
        elif mean_function.lower() == "density_nonlinear":
                mean_function = density_nonlinear(
                A=np.zeros(4).reshape(-1,1)
            )
        else:
            raise ValueError(
...
class sigma_epsilon_nonlinear(MeanFunction):
    ...
    def __call__(self, X) -> tf.Tensor:
        ...
        # Modified here, should be the same, test needed
        x_train_reshape = tf.concat([X[:,0:1]*X[:,2:3],X[:,1:2]*X[:,3:4],X[:,4:5]],axis=1)
...
class density_nonlinear(MeanFunction):
    """
    y_i = A x_i + b
    x_1 = x1^-3
    x_2 = x2^-3
    ...
    x_4 = x4/x5
    only allowed 5 columns input for x
    notice that A here request 4 coefficients
    """
    ...
    def __call__(self, X) -> tf.Tensor:
        x_train_reshape = tf.concat([X[:,0:1]**(-3),X[:,1:2]**(-3),X[:,2:3]/X[:,4:5],X[:,3:4]/X[:,4:5]],axis=1)
        return tf.tensordot(x_train_reshape, self.A, [[-1], [0]]) + self.b