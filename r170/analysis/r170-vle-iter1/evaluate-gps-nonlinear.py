...
pdf = PdfPages('figs/gp_models_nonlinear.pdf')
...
in_csv_names = ["r170-vle-iter" + str(i) + "-results.csv" for i in range(1, iternum+1)]
...
models["Matern32"] = run_gpflow_scipy(
    x_train,
    y_train,
    gpflow.kernels.Matern32(lengthscales=np.ones(R170.n_params + 1)),
    #mean_function="sigma_epsilon_nonlinear"
)
...
pdf.savefig(plot_model_performance(models, x_train, y_train, R170.liq_density_bounds))
pdf.savefig(plot_model_performance(models, x_test, y_test, R170.liq_density_bounds))

# Plot temperature slices
figs = plot_slices_temperature(
    models,
    R170.n_params,
    R170.temperature_bounds,
...
for param_name in R170.param_names:
...
for test_params in x_test[:,:R170.n_params]:
    train_points = []
    test_points = []
    # Locate rows where parameter set == test parameter set
    matches = np.unique(np.where((df_all[list(R170.param_names)] == test_params).all(axis=1))[0])
    ...
    for match in matches:
        ...
        if np.where((df_all.values[match,:R170.n_params+1] == x_test[:,:R170.n_params+1]).all(axis=1))[0].shape[0] == 1:
...
# Plot model performance on train and test points
pdf.savefig(plot_model_performance(models, x_train, y_train, R170.vap_density_bounds))
pdf.savefig(plot_model_performance(models, x_test, y_test, R170.vap_density_bounds))
...
param_names = list(R170.param_names) + ["temperature_pvap"]
...
# Plot model performance on train and test points
pdf.savefig(plot_model_performance(models, x_train, y_train, R170.Pvap_bounds))
pdf.savefig(plot_model_performance(models, x_test, y_test, R170.Pvap_bounds))
...
# Plot model performance on train and test points
pdf.savefig(plot_model_performance(models, x_train, y_train, R170.Hvap_bounds))
pdf.savefig(plot_model_performance(models, x_test, y_test, R170.Hvap_bounds))
