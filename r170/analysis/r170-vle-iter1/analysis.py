...
def main():
    ...
    plot_property(
        df_all, "Pvap", R170.Pvap_bounds, axis_name="Vapor Pressure [bar]"
    )
    plot_property(
        df_all,
        "Hvap",
    ...
    d = pd.DataFrame({'T':list(values_scaled_to_real(df_all["temperature"],R170.temperature_bounds)) , 'sim_Pvap':list(values_scaled_to_real(df_all["sim_Pvap"], R170.Pvap_bounds)) }, columns=['T', 'sim_Pvap'])
    #print(d.loc[d['T'] == 240])
    ...
    temps = R170.expt_liq_density.keys()
    for temp in temps:
        ax.scatter(
    ...
    ax.scatter(R170.expt_rhoc, R170.expt_Tc, color="black", marker="x", s=80)
    ax.scatter(
        R170.trappe_liq_density.values(),
        R170.trappe_liq_density.keys(),
        color="red",
        marker="x",
        s=80,
    )
    ax.scatter(
        R170.trappe_vap_density.values(),
        R170.trappe_vap_density.keys(),
        color="red",
        marker="x",
        s=80,
    )
    ax.scatter(R170.trappe_rhoc, R170.expt_Tc, color="red", marker="x", s=80)
    ...
    ax.scatter(
        R170.trappe_Pvap.keys(),
        R170.trappe_Pvap.values(),
        color="red",
        marker="x",
        s=80,
    )
