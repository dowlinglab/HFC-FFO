...
def main():
    ...
    plot_property(
        df_all,
        "vap_density",
        R50.vap_density_bounds,
        axis_name="Vapor Density [kg/m$^3$]",
    )
    plot_property(
        df_all, "Pvap", R50.Pvap_bounds, axis_name="Vapor Pressure [bar]"
    )
    plot_property(
        df_all,
        "Hvap",
    ...
    d = pd.DataFrame({'T':list(values_scaled_to_real(df_all["temperature"],R50.temperature_bounds)) , 'sim_Pvap':list(values_scaled_to_real(df_all["sim_Pvap"], R50.Pvap_bounds)) }, columns=['T', 'sim_Pvap'])
    #print(d.loc[d['T'] == 240])
    # Create a dataframe with one row per parameter set
    df_paramsets = prepare_df_vle_errors(df_all, R50)
    ...
    temps = R50.expt_liq_density.keys()
    for temp in temps:
        ax.scatter(
    ...
    ax.scatter(R50.expt_rhoc, R50.expt_Tc, color="black", marker="x", s=80)
