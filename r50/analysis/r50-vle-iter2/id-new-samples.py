...
distance_seed = 10
liquid_density_threshold = 200  # kg/m^3


# Read VLE files
csv_path = "/scratch365/nwang2/ff_development/HFC_143a_FFO_FF/r50/analysis/csv/"
in_csv_names = [
    "r50-vle-iter" + str(i) + "-results.csv" for i in range(1, iternum + 1)
]
...
df_vle = prepare_df_vle(df_csv, R50)

# Read liquid density files
max_density_iter = 4
in_csv_names = [
    "r50-density-iter" + str(i) + "-results.csv"
    for i in range(2, max_density_iter + 1)
]
df_csvs = [
    pd.read_csv(csv_path + in_csv_name, index_col=0)
    for in_csv_name in in_csv_names
...
df_test_points = df_vle[
    list(R50.param_names) + ["temperature", "sim_liq_density"]
]
xx = df_test_points[list(R50.param_names) + ["temperature"]].values
...
) - values_scaled_to_real(means, R50.liq_density_bounds)
print(
...
ranked_samples = rank_samples( # compare and downselect with MD density results first
    latin_hypercube, md_model, R50, "sim_liq_density", property_offset=13.5
)
...
print(
    ...
    "viable parameter sets which are within 25 kg/m$^3$ of GROMACS liquid densities",
)
...
for property_name, model in vle_models.items():
    vle_predicted_mses[property_name] = rank_samples(
        viable_samples, model, R50, property_name
    )
print("Completed calculating other properties!")

# Merge into single DF
vle_mses = vle_predicted_mses["sim_liq_density"].merge(
    vle_predicted_mses["sim_vap_density"], on=R50.param_names
)
vle_mses = vle_mses.rename(
    {"mse_x": "mse_liq_density", "mse_y": "mse_vap_density"}, axis="columns"
)
vle_mses = vle_mses.merge(vle_predicted_mses["sim_Pvap"], on=R50.param_names)
vle_mses = vle_mses.merge(vle_predicted_mses["sim_Hvap"], on=R50.param_names)
vle_mses = vle_mses.rename(
...
result, pareto_points, dominated_points = find_pareto_set(
    vle_mses.drop(columns=list(R50.param_names)).values, is_pareto_efficient
)
...
g.savefig("figs/R50-pareto-mses.pdf")

# Plot pareto points vs. params
g = seaborn.pairplot(vle_mses, vars=list(R50.param_names), hue="is_pareto")
g.set(xlim=(-0.1, 1.1), ylim=(-0.1, 1.1))
g.savefig("figs/R50-pareto-params.pdf")
