...
def run_gpflow_scipy(x_train, y_train, kernel, mean_function="linear", fmt="notebook"):
    ...
    if mean_function is not None:
        if mean_function == "linear":
            ...
            mean_function = None
        elif mean_function.lower() == "nonlinear1":
...
class nonlinear1(MeanFunction):
    ...
    def __init__(self, A: TensorType = None, b: TensorType = None) -> None:
        ...
        A = tf.ones((1, 1), dtype=default_float()) if A is None else A
        b = tf.zeros(1, dtype=default_float()) if b is None else b
