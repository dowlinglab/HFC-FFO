...
def main():
    ...
    for temp in temps_r50:
        ax2.scatter(
            df_r50.loc[f"sim_liq_density_{float(temp):.0f}K"],
            temp,
        ...
        ax2.scatter(
            df_r50.loc[f"sim_vap_density_{float(temp):.0f}K"],
            temp,
    ...
    tc, rhoc = calc_critical(df_r50_trappe)
    print("TraPPE: ", tc, " K ",rhoc)
    ax2.scatter(
        df_r50_trappe["liq_density"],
...
'''    # Plot Pvap / Hvap
    ...
    axs[0,1].text(0.5, 0.8, "HFC-50", fontsize=16, transform=axs[0,1].transAxes)
