...
dfs = [prepare_df_vle(df_csv, R50) for df_csv in df_csvs]


def main():

    # Create a dataframe with one row per parameter set
    dfs_paramsets = [prepare_df_vle_errors(df, R50) for df in dfs]
    ...
    text.set_y(3)
    axes[0].text(16,18,r"R-50",fontsize=16)
    ...
    fig.savefig("pdfs/fig2_r50-cumu-vle.png",dpi=300)
