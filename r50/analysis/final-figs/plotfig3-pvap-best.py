...
def main():
    ...
    for temp in temps_r50:
        axs[0].scatter(
            temp,
            df_r50.loc[f"sim_Pvap_{float(temp):.0f}K"],
            #np.tile(temp, len(df_r50)),
    ...
    for temp in temps_r50:
        axs[1].scatter(
            temp,
            df_r50.loc[f"sim_Hvap_{float(temp):.0f}K"],
            #np.tile(temp, len(df_r50)),
    ...
    print(df_r50_gaff["temperature"],df_r50_gaff["Hvap"] / R50.molecular_weight * 1000.0)
    axs[1].scatter(
        df_r50_lit["temperature"],
