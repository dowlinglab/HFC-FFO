...
def init_project():
    ...
    # Define temps
    temps = [210.0 * u.K, 230.0 * u.K, 250.0 * u.K, 270.0 * u.K, 290.0 * u.K]
    ...
    lh_samples = np.genfromtxt(
        "../../analysis/csv/r41-vle-iter2-params.csv",
        delimiter=",",
        skip_header=1,
    )[:, 1:]
    ...
    for temp in temps:
        for sample in scaled_params:
            ...
            state_point = {
                ...
                "N_vap": n_vap,
                "N_liq": n_liq,
                "expt_liq_density": R41.expt_liq_density[int(temp.in_units(u.K).value)],
                "nsteps_liqeq": 5000,
                "nsteps_eq": 10000,
                "nsteps_prod": 100000,
            }
