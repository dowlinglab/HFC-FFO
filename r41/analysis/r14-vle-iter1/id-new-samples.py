...
df_vle = prepare_df_vle(df_csv, R14)

# Read liquid density files
max_density_iter = 4
in_csv_names = [
    "r14-density-iter" + str(i) + "-results.csv"
...
df_test_points = df_vle[
    list(R14.param_names) + ["temperature", "sim_liq_density"]
]
xx = df_test_points[list(R14.param_names) + ["temperature"]].values
...
) - values_scaled_to_real(means, R14.liq_density_bounds)
print(
...
ranked_samples = rank_samples( # compare and downselect with MD density results first
    latin_hypercube, md_model, R14, "sim_liq_density", property_offset=13.5
)
...
for property_name, model in vle_models.items():
    vle_predicted_mses[property_name] = rank_samples(
        viable_samples, model, R14, property_name
    )
print("Completed calculating other properties!")

# Merge into single DF
vle_mses = vle_predicted_mses["sim_liq_density"].merge(
    vle_predicted_mses["sim_vap_density"], on=R14.param_names
)
vle_mses = vle_mses.rename(
    {"mse_x": "mse_liq_density", "mse_y": "mse_vap_density"}, axis="columns"
)
vle_mses = vle_mses.merge(vle_predicted_mses["sim_Pvap"], on=R14.param_names)
vle_mses = vle_mses.merge(vle_predicted_mses["sim_Hvap"], on=R14.param_names)
vle_mses = vle_mses.rename(
...
result, pareto_points, dominated_points = find_pareto_set(
    vle_mses.drop(columns=list(R14.param_names)).values, is_pareto_efficient
)
...
g.savefig("figs/R14-pareto-mses.pdf")

# Plot pareto points vs. params
g = seaborn.pairplot(vle_mses, vars=list(R14.param_names), hue="is_pareto")
g.set(xlim=(-0.1, 1.1), ylim=(-0.1, 1.1))
g.savefig("figs/R14-pareto-params.pdf")
