...
param_csv_names = [
    "r32-density-iter" + str(i) + "-params.csv" for i in range(1, iternum + 1)
]
result_csv_names = [
...
df_results["expt_density"] = df_results["temperature"].apply(
    lambda x: R32.expt_liq_density[int(x)]
)
df_results["sq_err"] = (df_results["density"] - df_results["expt_density"]) ** 2
df_mse = (
    df_results.groupby(list(R32.param_names))["sq_err"].mean().reset_index(name="mse")
)

scaled_param_values = values_real_to_scaled(
    df_mse[list(R32.param_names)], R32.param_bounds
)
param_idxs = []
param_vals = []
for params1 in scaled_param_values:
    for idx, params2 in enumerate(df_params[list(R32.param_names)].values):
        if np.allclose(params1, params2):
            param_idxs.append(idx)
            param_vals.append(params2)
            break
df_mse["param_idx"] = param_idxs
df_mse[list(R32.param_names)] = param_vals

# Plot all with MSE < 625
g = seaborn.pairplot(
    pd.DataFrame(df_mse[df_mse["mse"] < 625.0], columns=list(R32.param_names))
)
g.set(xlim=(-0.1, 1.1), ylim=(-0.1, 1.1))
g.savefig("R32-all-MSE.lt.625.pdf")
# Plot all with MSE < 100
g = seaborn.pairplot(
    pd.DataFrame(df_mse[df_mse["mse"] < 100.0], columns=list(R32.param_names))
)
g.set(xlim=(-0.1, 1.1), ylim=(-0.1, 1.1))
g.savefig("R32-all-MSE.lt.100.pdf")
...
while len(top_param_set > 0):
    ...
    l1_norm = np.sum(
        np.abs(
            top_param_set[list(R32.param_names)].values
            - final_param_set[list(R32.param_names)].iloc[[-1]].values
        ),
        axis=1,
    )
...
g = seaborn.pairplot(
    pd.DataFrame(final_param_set_mse625, columns=list(R32.param_names))
)
g.set(xlim=(-0.1, 1.1), ylim=(-0.1, 1.1))
g.savefig("R32-greedy-MSE.lt.625.pdf")

g = seaborn.pairplot(
    pd.DataFrame(final_param_set_mse100, columns=list(R32.param_names))
)
g.set(xlim=(-0.1, 1.1), ylim=(-0.1, 1.1))
