...
def init_project():
    ...
    # Initialize project
    project = signac.init_project("r32-density-iter1")
    ...
    lh_samples = np.genfromtxt("data/lh_samples_200_r32.txt")

    # Define bounds on sigma/epsilon
    bounds_sigma = np.asarray([[3.0, 4.0], [2.5, 3.5], [1.7, 2.7],])  # C  # F  # H
    ...
    for temp in temps:
        for sample in scaled_params:
            ...
            # Unpack the sample
            (sigma_C, sigma_F, sigma_H, epsilon_C, epsilon_F, epsilon_H) = sample
            ...
            state_point = {
                ...
                "epsilon_C": float((epsilon_C * u.K * u.kb).in_units("kJ/mol").value),
                "epsilon_F": float((epsilon_F * u.K * u.kb).in_units("kJ/mol").value),
                "epsilon_H": float((epsilon_H * u.K * u.kb).in_units("kJ/mol").value),
            }
