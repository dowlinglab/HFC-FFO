...
csv_path = "../csv/"
in_csv_names = ["r14-density-iter" + str(i) + "-results.csv" for i in range(1, iternum+1)]
out_csv_name = "r14-density-iter" + str(iternum + 1) + "-params.csv"
out_top_liquid_csv_name = "r14-density-iter" + str(iternum ) + "-liquid-params.csv"
out_top_vapor_csv_name = "r14-density-iter" + str(iternum ) + "-vapor-params.csv"
...
ranked_liquid_samples = rank_samples(liquid_samples, model, R14, "sim_liq_density")
ranked_vapor_samples = rank_samples(vapor_samples, model, R14, "sim_liq_density")#both l and g compared to liquid density
...
column_names = list(R14.param_names)
...
dist_seed = 115
#target_num_v = 48
target_num_l = 201 # just for liquid; vapor less than 100 and use all
...
#distance_opt_v,number_points_v = bisection(lower_bound, upper_bound, error_tol, top_vap, R14, target_num_v, dist_seed)
#print('\nRequired Distance for vapor is : %0.8f and there are %0.1f points too few' % (distance_opt_v, number_points_v) )

distance_opt_l,number_points_l = bisection(lower_bound, upper_bound, error_tol, top_liq, R14, target_num_l, dist_seed)
print('\nRequired Distance for liquid is : %0.8f and there are %0.1f points too few' % (distance_opt_l, number_points_l) )

new_points_l = opt_dist(distance_opt_l, top_liq, R14, target_num_l, rand_seed=dist_seed , eval = True)
...
#new_points_v = opt_dist(distance_opt_v, top_vap, R14, target_num_v, rand_seed=dist_seed , eval = True)
