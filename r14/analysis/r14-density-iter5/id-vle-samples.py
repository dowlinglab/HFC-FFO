...
liquid_density_threshold = 1100  # kg/m^3  ##>140 is liquid; <140 is gas. used for classifier

csv_path = "/scratch365/nwang2/ff_development/HFC_143a_FFO_FF/r14/analysis/csv/"

# Load in all parameter csvs and result csvs
param_csv_names = [
    "r14-density-iter" + str(i) + "-params.csv" for i in range(1, iternum + 1)
]
print(param_csv_names)
result_csv_names = [
    "r14-density-iter" + str(i) + "-results.csv" for i in range(1, iternum + 1)
]
df_params = [
...
df_results["expt_density"] = df_results["temperature"].apply(
    lambda x: R14.expt_liq_density[int(x)]
)
df_results["sq_err"] = (df_results["density"] - df_results["expt_density"]) ** 2
df_mse = (
    df_results.groupby(list(R14.param_names))["sq_err"].mean().reset_index(name="mse")
)
scaled_param_values = values_real_to_scaled(
    df_mse[list(R14.param_names)], R14.param_bounds
)
param_idxs = []
param_vals = []
for params1 in scaled_param_values:
    for idx, params2 in enumerate(df_params[list(R14.param_names)].values):
        if np.allclose(params1, params2):
            param_idxs.append(idx)
            param_vals.append(params2)
            break
df_mse["param_idx"] = param_idxs
df_mse[list(R14.param_names)] = param_vals

top_param_set = df_mse[df_mse["mse"] < 196]
print(top_param_set)
...
distance_opt_l,number_points_l = bisection(lower_bound, upper_bound, error_tol, top_param_set, R14, target_num_l, dist_seed)
print('\nRequired Distance for liquid is : %0.8f and there are %0.1f points too few' % (distance_opt_l, number_points_l) )

new_points_vle = opt_dist(distance_opt_l, top_param_set, R14, target_num_l, rand_seed=dist_seed , eval = True)
...
new_points_vle.to_csv(csv_path + "r14-vle-iter1-params.csv")
